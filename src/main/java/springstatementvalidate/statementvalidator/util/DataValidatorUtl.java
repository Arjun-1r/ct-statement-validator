	...
	public static Long validateTransacNumber(String ref) {
		if (TRANSACTION_REF_PATTERN.matcher(ref).matches()) {
			...
			throw new ValidationException(
					String.format("Transaction Reference '%s' is not valid. Should only contains numeric value.", ref));
		}
	}
	...
	public static String validateAndGetAccountNumber(String accountNumber) {
		if (ACCOUNT_NUMBER_PATTERN.matcher(accountNumber).matches()) {
			...
			throw new ValidationException(String.format(
					"Account Number '%s' is not valid. Should have this [A-Z]{2}[0-9]{2}[A-Z]{4}[0-9]{10} format.",
					accountNumber));
		}
	}
